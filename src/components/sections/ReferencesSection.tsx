...
export function ReferencesSection() {
  return (
    <section className="mx-auto max-w-7xl px-6 py-20">
      ...
      <div className="mt-10 grid grid-cols-2 gap-6 opacity-70 sm:grid-cols-4">
        {Array.from({ length: PLACEHOLDER_COUNT }).map((_, index) => (
          <div
            key={index}
            className="flex h-16 items-center justify-center rounded-lg border border-slate-100 bg-white text-xs font-medium text-slate-400"
          >
            LOGO
          </div>
        ))}
